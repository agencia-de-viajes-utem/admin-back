...
func ConfigureHotelRouter(router *gin.Engine, authService *services.AuthService) {
	...
	hotelGroup.GET("/get/:id", func(c *gin.Context) {
		...
		// Transformar los datos a models.HabitacionSinHotel
		var habitacionesResp []models.HabitacionSinHotel
		for _, h := range habitaciones {
			habResp := models.HabitacionSinHotel{
				ID:               h.ID,
				Nombre:           h.Nombre,
				...
				Ocupada:          h.Ocupada,
				IDPaquete:        h.IDPaquete,
			}
			habitacionesResp = append(habitacionesResp, habResp)
		}

		// Responder con la información de HotelHabitaciones
		c.JSON(http.StatusOK, habitacionesResp)
	})
