...
func ConfigurePaqueteRoutes(router *gin.Engine, authService *services.AuthService) {
	...
	// Ruta para GET (obtener un paquete por ID)
	paqueteGroup.GET("/get/:id", func(c *gin.Context) {
		// Obtener el ID del paquete desde la URL
		paqueteIDStr := c.Param("id")
		...
		// Obtener el paquete desde la base de datos
		paquete, err := services.GetPaqueteByID(authService.DB, paqueteID)
		if err != nil {
			log.Printf("Error al obtener el paquete: %v\n", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error al obtener el paquete"})
			return
		}

		// Transformar el paquete a su versión resumida
		paqueteResumido := utils.TransformarAPaqueteResumido(*paquete)

		// Responder con el paquete resumido
		c.JSON(http.StatusOK, paqueteResumido)
	})
	...
	paqueteGroup.DELETE("/delete-relations", middleware.AdminAuthMiddleware(), func(c *gin.Context) {
		// Obtener el ID del paquete a eliminar relaciones desde la solicitud
		paqueteIDStr := c.Query("id")
		...
		// Eliminar las relaciones paquetes_habitaciones de la base de datos
		if err := services.DeletePaquetesHabitacionesByPaqueteID(authService.DB, paqueteID); err != nil {
			log.Printf("Error al eliminar las relaciones paquetes_habitaciones: %v\n", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error al eliminar las relaciones paquetes_habitaciones"})
			return
		}

		// Responder con un código de estado 200 (OK)
		log.Printf("Relaciones paquetes_habitaciones eliminadas correctamente\n")
		c.JSON(http.StatusOK, gin.H{"message": "Relaciones paquetes_habitaciones eliminadas correctamente"})
	})
